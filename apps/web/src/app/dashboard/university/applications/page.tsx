...
export default function ApplicationsPage() {
  ...
  useEffect(() => {
    ...
    const KEY =
      "insa_univ_apps_rendered_" + Math.random().toString(36).slice(2);
    ...
    return () => {
      (window as any)[STORAGE_KEY] = false;
    };
  }, []);
  ...
  const [officialLetterFile, setOfficialLetterFile] = useState<File | null>(
    null,
  );
  ...
  const handleCreateApplication = async () => {
    ...
    if (user?.role !== "UNIVERSITY") {
      toast.error("Only university accounts can create applications");
      return;
    }
    if (!user?.university?.id) {
      toast.error(
        "Your account is not linked to a university. Please contact an admin.",
      );
      return;
    }
    ...
    if (!officialLetterFile) {
      toast.error("Official letter file is required");
      return;
    }
    ...
    try {
      let uploadRes;
      try {
        uploadRes = await uploadDocument(
          officialLetterFile,
          {
            type: "OFFICIAL_LETTER",
            title: `Official Letter - ${formData.academicYear}`,
          },
          token || undefined,
        );
      } catch (uploadErr: any) {
      ...
      try {
        ...
        if (status === 400) {
          toast.error(
            "Creation failed: Backend rejected the request. Please ensure all data is valid.",
          );
          return;
        }
        throw createErr;
      ...
      if (status === 403) {
        toast.error(
          "Forbidden: ensure you are logged in as a university and your account is linked to a university",
        );
      } else {
        toast.error(error?.message || "Failed to complete application process");
      }
    }
  };
  ...
  const handleSubmitForReview = async (app: Application) => {
    ...
    if (!canSubmit) {
      toast.error(
        "Cannot send for review: ensure status PENDING, at least 1 student, and official letter uploaded.",
      );
      return;
    }
    try {
      await submitApplication(app.id, token || undefined);
      toast.success("Application sent for review");
      fetchApplications();
    } catch (error: any) {
      toast.error(error?.message || "Failed to send application for review");
    }
  };
  ...
  const filteredApplications = uniqueApplications.filter(
  ...
  const stats = {
    ...
    underReview: uniqueApplications.filter((a) => a.status === "UNDER_REVIEW")
      .length,
    ...
    <div className="space-y-8 px-4">
      <div className="rounded-2xl border bg-card p-6 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Create, track, and modify batches of student internship requests
            </p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <Button
              ...
              size="lg"
              className="gap-2"
              disabled={!token || user?.role !== "UNIVERSITY"}
            >
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          <Card className="border-l-4 border-l-foreground/70 bg-background shadow-sm">
          ...
          <Card
            className="border-l-4 border-l-warning cursor-pointer bg-background shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          ...
          <Card
            className="border-l-4 border-l-secondary cursor-pointer bg-background shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          ...
          <Card
            className="border-l-4 border-l-success cursor-pointer bg-background shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          ...
          <Card
            className="border-l-4 border-l-destructive cursor-pointer bg-background shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          ...
          <div className="space-y-3">
            {filteredApplications.length === 0 ? (
              ...
              filteredApplications.map((app) => {
                ...
                const letterUrl =
                  app.officialLetterUrl || (app as any).official_letter_url;
                const canSubmit =
                  app.status === "PENDING" &&
                  !!letterUrl &&
                  app.studentCount > 0;
                ...
                return (
                  <div
                    key={app.id}
                    className="flex flex-col gap-4 rounded-xl border bg-background p-5 transition hover:-translate-y-0.5 hover:shadow-md md:flex-row md:items-center md:justify-between"
                  >
                    <div className="flex items-center gap-4 flex-1">
                      <div
                        className={`p-4 rounded-xl bg-gradient-to-br from-${statusConfig.variant}/20 to-${statusConfig.variant}/10 shadow-sm`}
                      >
                        <StatusIcon
                          className={`h-6 w-6 text-${statusConfig.variant}`}
                        />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          <h3 className="text-lg font-bold">
                            {(app as any).name || "Untitled Application"}
                          </h3>
                          <Badge
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {letterUrl && isSafeHttpUrl(letterUrl) && (
                        <a
                          href={sanitizeUrl(letterUrl)}
                          target="_blank"
                      ...
                      {(app.status === "PENDING" ||
                        ...
                        <Button
                          ...
                          onClick={() =>
                            router.push(
                              `/dashboard/university/students?applicationId=${app.id}`,
                            )
                          }
                        >
                          <Users className="h-4 w-4" />
                        ...
                        <>
                          ...
                          {canSubmit ? (
                            ...
                            <Button
                              ...
                              title={
                                app.status !== "PENDING"
                                  ? `Status is ${app.status}, not Draft`
                                  : !letterUrl
                                    ? "Official letter missing"
                                    : app.studentCount === 0
                                      ? "Add at least 1 student"
                                      : `Requirements not met (Count: ${app.studentCount}, Letter: ${!!letterUrl})`
                              }
                            >
      ...
      <Dialog open={showNewAppDialog} onOpenChange={setShowNewAppDialog}>
        <DialogContent className="sm:max-w-[480px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Start a new internship application cycle. You'll be able to add
              students after creation.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input
                id="app-name"
                placeholder="e.g., Software Engineering Batch 2025"
                value={formData.name}
                onChange={(e) => {
            ...
            <div className="space-y-2">
              ...
              <div className="border-2 border-dashed rounded-xl p-4 text-center hover:bg-muted/30 transition-colors">
                <input
                  ...
                  className="hidden"
                  onChange={(e) =>
                    setOfficialLetterFile(e.target.files?.[0] || null)
                  }
                />
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !formData.name || !formData.academicYear || !officialLetterFile
              }
            >
              Create Application
            </Button>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Update application: {selectedApp?.name || selectedApp?.id}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                id="edit-name"
                placeholder="e.g., Fall Internship Batch 2024"
                value={formData.name}
                onChange={(e) =>
                  setFormData({ ...formData, name: e.target.value })
                }
              ...
              <p className="text-xs text-muted-foreground">
                Short name to identify this application
              </p>
            </div>
            <div className="space-y-2">
                ...
                <CardContent className="p-6 text-center">
                  ...
                  <label htmlFor="edit-letter" className="cursor-pointer">
                    ...
                    {officialLetterFile ? (
                      ...
                      <p className="text-sm text-muted-foreground">
                        Current file:{" "}
                        {selectedApp.officialLetterUrl.split("/").pop()}
                      </p>
                    ) : (
                      <p className="text-sm text-muted-foreground">
                        Click to upload
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdateApplication}>Save Changes</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showViewDialog} onOpenChange={setShowViewDialog}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Complete information for {selectedApp?.name || selectedApp?.id}
            </DialogDescription>
          </DialogHeader>
          {selectedApp && (
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium mt-1 uppercase">
                    {selectedApp.name || "Untitled Application"}
                  </p>
                </div>
                <div>
              ...
              {selectedApp.officialLetterUrl &&
                isSafeHttpUrl(selectedApp.officialLetterUrl) && (
                  <div>
                    <Label className="text-muted-foreground">
                      Official Letter
                    </Label>
                    <div className="mt-2">
                      <a
                        href={sanitizeUrl(selectedApp.officialLetterUrl)}
                        target="_blank"
                        rel="noopener noreferrer"
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              This will archive the application. You can restore it later by
              contacting an admin.
            </DialogDescription>
          </DialogHeader>
          <div className="py-2">
            <p className="text-sm">
              Application: {selectedApp?.name || selectedApp?.id}
            </p>
          </div>
          <DialogFooter>
            <Button
