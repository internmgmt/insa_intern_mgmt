...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    void fetchUsers();
  }, [page, selectedUniversityId, selectedDepartmentId]);
  ...
  async function fetchUsers() {
    try {
      ...
      const totalPages =
        (res as any)?.data?.pagination?.totalPages ??
        (res as any)?.pagination?.totalPages ??
        1;
      setUsers(items);
  ...
  async function fetchResources() {
    try {
      ...
      setUniversities(
        ((uniRes as any)?.data?.items ?? (uniRes as any)?.data ?? []).map(
          (u: any) => ({ id: u.id, name: u.name }),
        ),
      );
      setDepartments(
        ((deptRes as any)?.data?.items ?? (deptRes as any)?.data ?? []).map(
      ...
      await createUser(
        {
          ...
          universityId:
            newUser.role === "UNIVERSITY"
              ? newUser.universityId || undefined
              : undefined,
          departmentId:
            newUser.role === "SUPERVISOR"
              ? newUser.departmentId || undefined
              : undefined,
        },
        token || undefined,
      );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Scope by university or department first, then search within the visible set."
        right={
          <Button
      ...
      {showNewUserForm && (
        <Card className="border-primary/20 bg-primary/2">
          <CardHeader>
            ...
            <CardDescription>
              Accounts are created with an auto-generated temporary password
      ...
      <Card className="border-dashed border-primary/20 bg-muted/20">
        <CardContent className="grid gap-4 py-4 md:grid-cols-[1.1fr_1fr_auto]">
          ...
          <div className="flex items-end justify-start md:justify-end">
            <Button variant="outline" onClick={resetFilters} className="h-9">
              <Filter className="mr-2 h-4 w-4" /> Reset scopes
            </Button>
          </div>
        </CardContent>
      </Card>
                ...
                users.map((u) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge
                        variant="secondary"
                        className="px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-tight"
                      >
                        {u.role}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <div
                          className={`h-2 w-2 rounded-full ${u.isActive ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-rose-500"}`}
                        />
                    ...
                    <TableCell className="text-right pr-6">
                      ...
                      >
                        {u.isActive ? (
                          ...
                          <>
                            <Power className="h-4 w-4 mr-2" /> Restore
                          </>
                        )}
                      </Button>
                    </TableCell>
