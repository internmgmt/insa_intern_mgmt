...
export async function listInterns(
  ...
  return apiFetch<ApiSuccess<Paginated<InternListItem>>>(
    `/interns${query ? `?${query}` : ""}`,
    { method: "GET", token },
  );
}
...
export async function createIntern(
  ...
  return apiFetch<ApiSuccess<InternListItem>>(`/interns`, {
    method: "POST",
    body,
    token,
  });
...
export async function getInternById(internId: string, token?: string) {
  return apiFetch<ApiSuccess<any>>(`/interns/${internId}`, {
    method: "GET",
    token,
  });
}
...
export async function updateIntern(
  internId: string,
  body: { assignedMentorId?: string | null; status?: string },
  token?: string,
) {
  return apiFetch<ApiSuccess<any>>(`/interns/${internId}`, {
    method: "PATCH",
...
export async function assignSupervisor(
  ...
  return apiFetch<ApiSuccess<InternListItem>>(
    `/interns/${internId}/assign-supervisor`,
    { method: "POST", body, token },
  );
}

export async function completeInternship(
  internId: string,
  body: { finalEvaluation: number; completionNotes?: string },
  token?: string,
) {
  return apiFetch<ApiSuccess<InternListItem>>(`/interns/${internId}/complete`, {
    method: "POST",
    body,
    token,
  });
...
export async function terminateInternship(
  internId: string,
  body: { reason: string },
  token?: string,
) {
  return apiFetch<ApiSuccess<InternListItem>>(
    `/interns/${internId}/terminate`,
    { method: "POST", body, token },
  );
}

export async function suspendIntern(
  internId: string,
  body: { reason: string },
  token?: string,
) {
  return apiFetch<ApiSuccess<InternListItem>>(`/interns/${internId}/suspend`, {
    method: "POST",
    body,
    token,
  });
...
export async function issueCertificate(
  ...
  return apiFetch<ApiSuccess<InternListItem>>(
    `/interns/${internId}/issue-certificate`,
    { method: "POST", body, token },
  );
}

export async function deleteIntern(internId: string, token?: string) {
  return apiFetch<ApiSuccess<null>>(`/interns/${internId}`, {
    method: "DELETE",
    token,
  });
}
