...
export async function listUniversities(
  ...
  return apiFetch<ApiSuccess<Paginated<University>>>(path, {
    method: "GET",
    token,
  });
}
...
export async function getUniversityById(id: string, token?: string) {
  return apiFetch<ApiSuccess<University>>(`/universities/${id}`, {
    method: "GET",
    token,
  });
}

export async function deleteUniversityPermanently(id: string, token?: string) {
  return apiFetch<ApiSuccess<null>>(`/universities/${id}/permanent`, {
    method: "DELETE",
    token,
  });
}
