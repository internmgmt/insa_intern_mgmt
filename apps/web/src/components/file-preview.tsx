...
export function FilePreview({ url, onClose, title }: FilePreviewProps) {
  ...
  useEffect(() => {
    ...
    const isProtected =
      currentUrl.startsWith("/api") || !currentUrl.startsWith("http");
      ...
      fetch(currentUrl, {
        ...
        .then(async (res) => {
          if (!res.ok) {
            ...
            const err = new Error(
              `Failed to load file (status=${res.status}) ${bodyText ? "- " + bodyText : ""}`,
            );
            // If this looks like the documents download endpoint and we got 401/403, try fetching metadata
            if (
              (res.status === 401 || res.status === 403) &&
              /\/api\/documents\/[0-9a-fA-F-]+\/download/.test(currentUrl)
            ) {
              try {
                const idMatch = currentUrl.match(
                  /\/api\/documents\/([0-9a-fA-F-]+)\/download/,
                );
                if (idMatch) {
                  ...
                  let infoBody = "";
                  try {
                    infoBody = await infoRes.text();
                  } catch (e) {}
                  console.warn(
                    `Document info response: status=${infoRes.status}`,
                    infoBody,
                  );
                }
              } catch (e) {
                console.warn(
                  "Failed to fetch document info for diagnostics",
                  e,
                );
              }
            }
        ...
        .catch((err) => {
          ...
          try {
            ...
            if (msg.includes("status=401")) {
              toast.error(
                "Unauthorized: your session may have expired. Please log out and log in again.",
              );
  ...
  const isImage =
    /\.(jpg|jpeg|png|gif|webp|svg)/i.test(currentUrl || "") ||
    currentUrl?.toLowerCase().includes("image") ||
          ...
          <div className="flex items-center gap-3 min-w-0 flex-1 mr-4">
            ...
            <div className="min-w-0 flex flex-col justify-center">
              ...
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium truncate">
                {urls.length > 1
                  ? `File ${activeIndex + 1} of ${urls.length}`
                  : "Document Viewer"}
              </p>
            </div>
          </div>
          ...
          <div className="w-full h-full flex items-center justify-center p-1 sm:p-4 overflow-auto">
            {loading ? (
              ...
              <div className="text-center p-10 max-w-sm mx-auto border border-dashed rounded-xl bg-muted/30">
                <File className="h-12 w-12 mx-auto text-muted-foreground/75 mb-4" />
